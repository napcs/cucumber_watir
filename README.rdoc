...
Devkit: http://wiki.github.com/oneclick/rubyinstaller/development-kit
  ...
  gem install cucumber rspec selenium-webdriver win32-process --no-ri --no-rdoc
...
=== Mac
  ...
  sudo gem install cucumber safariwatir rspec selenium-webdriver --no-ri --no-rdoc
...
On Windows, it will use IE by default. On the Mac, it'll use Firefox. You can override this
  ...
  rake features BROWSER=firefox
  rake features BROWSER=chrome
  
== Running a story

You can run a specific story with 

  rake features FEATURE=features/twitter.feature               
